...
module.exports = function create(fittingDef, bagpipes) {
  ...
  return function error_handler(context, next) {
    ...
    try {
      ...
      var argument = err.errors[0].errors[0].path[0];;
      var argumentErrorMsg = err.errors[0].errors[0].message;
      var errorCode = err.errors[0].errors[0].code;

      if(errorCode === 'OBJECT_MISSING_REQUIRED_PROPERTY') {
        var errMsg = `Invalid request. ${argumentErrorMsg}`;
      } else {
        var errMsg = `Invalid argument ${argument}. ${argumentErrorMsg}`;
      }
      ...
      next(null, JSON.stringify({'message': errMsg}));
    } catch (err2) {
      log = context.request && (
               context.request.log
